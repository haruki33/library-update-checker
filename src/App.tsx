...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="filters" aria-label="Release filters">
      ...
      <div className="library-filter"><span>Library</span><div className="library-options">{libraries.map((library) => <label className="checkbox" key={library} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', width: 'auto', gap: '2px', margin: 0, textAlign: 'left' }}><input type="checkbox" checked={filters.libraries.includes(library)} onChange={() => toggleLibrary(library)} /><span className="library-name">{library}</span></label>)}</div></div>
    </section>
